...
def plot_price_history(ax, prices, df):
    ...
    # Create a boolean mask for valid months
    valid_months_mask = year_month_series.str[-2:].astype(int).between(1, 12)
    
    # Use the valid months mask to filter the DataFrame
    prices = prices[valid_months_mask.values]
    
    # Convert the index to datetime and filter for dates after January 2018
...
def update(label):
    ...
    with plt.rc_context({'axes.titlepad': 20}):  # Adjust title padding
        ...
        plot_price_history(ax_line_district, df[df['year_month'] == month])
...
def main():
    ...
    for d in dirs:
        ...
        if os.path.exists(full_path):
            df_temp = pd.read_csv(full_path, index_col=False)
            df_temp['source_file'] = d  # Add the source directory as a column
            df_temp['Q'] = d[-1]
            dfs.append(df_temp.iloc[1:])
        else:
            print(f"File not found: {full_path}")
    ...
    # Make sure the '交易年月日' column is a string
    df['交易年月日'] = df['交易年月日'].astype(str)

    # Now, create the 'year_month' column correctly
    df['year'] = df['交易年月日'].str[:-4].astype(int) + 1911
    df['month'] = df['交易年月日'].str[-4:-2].astype(int)
    df['year_month'] = df['year'].astype(str) + df['month'].apply(lambda x: '{:02d}'.format(x))
    ...
    # Group by 'year_month' and '鄉鎮市區' to get the mean '單價元坪', then unstack to get districts as columns
    prices_year_month = df.groupby(['year_month', '鄉鎮市區'])['單價元坪'].mean().unstack()
   ...
   # Create separate figures for each plot
    ...
    # Remove invalid months (e.g., month 00)
    df = df[(df['month'] >= 1) & (df['month'] <= 12)]
    # Initial plots
    with plt.rc_context({'axes.titlepad': 20}):  # Adjust title padding
        plot_year_histogram(2018, df, fig_hist, ax_hist)  # Initial plot for 2018
        plot_price_history(ax_line_district, prices_year_month,df)
